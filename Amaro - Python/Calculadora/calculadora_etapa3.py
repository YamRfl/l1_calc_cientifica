...
def dividir(n1, n2):
    """Retorna o queociente da divisão de dois números."""
    return n1 / n2

try: #comando para tratar exceções no código. Para lidar com erros no programa
    ...
    if not num1_str.replace('.', '', 1).isdigit() or not num2_str.replace('.', '', 1).isdigit():
        ...
        if operador in ['+', '-', '*', '/']:
            ...
            if operador == '+':
                ...
                if num2 == 0:
                    raise ZeroDivisionError("Não é possível dividir por zero.") #raise - permite  lançar uma exceção manualmente no código
                else:
