...
#Passo 2: Implementar funções para cláculos de potenciação, radiciação, trigonometria e logaritmos.
...
def radiciacao(numero, raiz):
    ...
    if numero < 0 and raiz % 2 == 0:
        raise ValueError("Raiz para de npumero negativo não é real.")
    return math.pow(numero, 1/raiz)

def seno(angulo_graus):
    """Retorna o seno de uma ângulo em graus."""
...
def logaritmo(numero, base):
    """Retorna o logartimo de um número na base especificada."""
    if numero <= 0 or base <= 0 or base == 1:
        raise ValueError("Número deve ser positivo e base diferente de 1.")
    return math.log(numero, base)

try:
    num1_str =  input("Digite o primeiro número: ")
    operador = input("Digite o operador (+, -, *, /, **, rad, sen, cos, tang, log): ")
    ...
    if operador in ['+', '-', '*', '/', '**', 'rad', 'sen', 'cos', 'tan', 'log']:
        #Passo 3: Integrar as novas funcções ao programa.
        if operador == '=':
